    ...
    public partial class frmNomina : Form, INominaView
    {
        ...
        private void ConfigurarDataGridView()
        {
            try
            {
                ...
                dgvNomina.RowsAdded += DgvNomina_RowsAdded;
        ...
        private void DgvNomina_RowsAdded(object? sender, DataGridViewRowsAddedEventArgs e)
        {
            for (int i = e.RowIndex; i < e.RowIndex + e.RowCount; i++)
            {
                if (i < dgvNomina.Rows.Count)
                {
                    AgregarBotonesAcciones(i);
                }
            }
        ...
        {
            var botonesAEliminar = dgvNomina.Controls.Cast<Control>()
                .Where(c => c is Button && c.Tag != null && c.Tag.ToString()!.StartsWith("btn_nomina_"))
                .ToList();
            
            foreach (var boton in botonesAEliminar)
            {
                dgvNomina.Controls.Remove(boton);
                boton.Dispose();
            }
        }
        ...
        {
            var botonesAEliminar = dgvNomina.Controls.Cast<Control>()
                .Where(c => c.Tag != null && c.Tag.ToString()!.Contains($"_{rowIndex}_"))
                .ToList();
        ...
        private void ReposicionarBotonesAcciones()
        {
            for (int i = 0; i < dgvNomina.Rows.Count; i++)
            {
                ...
                var controles = dgvNomina.Controls.Cast<Control>()
                    .Where(c => c.Tag != null && c.Tag.ToString()!.Contains($"_{i}_"))
                    .ToList();
