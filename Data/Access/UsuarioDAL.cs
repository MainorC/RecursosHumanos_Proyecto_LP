...
.FirstOrDefault(u => u.NombreUsuario == nombreUsuario);
  ...
  return false;
   ...
   // Intentar verificar con BCrypt
            bool esValida = PasswordHelper.VerifyPassword(contrasena, usuario.Contrasena);
       
         // Si falla con BCrypt, verificar si la contraseña coincide directamente
   // (caso donde el hash está corrupto o es incompatible)
       if (!esValida && usuario.Contrasena == contrasena)
      {
     // La contraseña guardada ES la contraseña en texto plano
 // pero tiene un formato que parece hash (raro pero posible)
              esValida = true;
              // Rehashear correctamente
